...
router.get("/timetable", async (req, res, next) => {
  try {
    ...
    const timetable = classData.concat(roomData);
...
router.get("/timeTable/:day", async (req, res, next) => {
  try {
    const timeTable = await timeTableData.find({ day: req.params.day });
...
router.get("/timeTable/:classKey", async (req, res, next) => {
  try {
    const [classData, roomData] = await Promise.all([
      timeTableData.aggregate([
        {
...
router.get("/timeTable/:day/:classKey", async (req, res, next) => {
  try {
    const timeTable = await timeTableData.findOne({
      day: req.params.day,
...
router.get(
  "/attendance/stud-details/:id/:studentId",
  async (req, res, next) => {
    try {
